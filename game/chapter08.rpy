label chapter08:
    # Tarde da noite Aneti aparece no dormitório e chama Maria pra ir

    scene bg_d_red with slowFade

    Narrador "Uma dia depois..."

    scene bg_i_red with fade

    $renpy.pause(0.5)

    Desconhecido "Pss...!"

    $renpy.pause(1)

    Desconhecido "{size=+15}Psis...!{/size}"

    $renpy.pause(1)

    Desconhecido "{size=+30}Psis...!{/size}"

    Desconhecido "Maria!!!"

    Eu "Hum...{w} O que?"

    Alo "Maria!!!"

    Eu "Alo?"

    show c_alo at center with dissolve

    Alo "Sim."

    Eu "O que foi, o que você... que horas são?"

    Alo "Bem tarde."

    Pensamento "???"

    Eu "E o que você quer a essas horas?{w} Não me diga que vai sair de novo?"

    Eu "Já vou te dizer que não vou ficar te cobrindo como fiz da ultima vez, a professora quase comeu meu figado."

    Alo "Não{w}. Não é isso...{w} Ou melhor...{w} é quase isso, mas de qualquer forma, se vista."

    Eu "Que, eu?"

    Alo "Sim, todo mundo já tá esperando."

    Eu "Que? Todo mundo?{w} Todo mundo quem?"

    Alo "Só não enrola, eu vou estar esperando lá fora."

    scene bg_d_red with slowFade

    Narrador "Algum tempo depois..."

    $renpy.pause(0.5)

    Pensamento "Eu vou matar a Alo se não for importante."

    Eu "Mas onde é que ela tá?"

    Alo "Maria!{w} Aqui!"

    # Está ali
    #     Aneti
    #     Hina
    #     Sophia
    #     Nikka
    #     Alo

    Eu "Ah, ai tá você..."

    Aneti "Iai Maria!"

    show c_aneti at center with dissolve

    Lemu "..." 

    Hina "Olá."

    show c_hina at left with dissolve

    Sophia "Oi Maria!"

    show c_sophia at right with dissolve

    Nikka "Oi Amiga!"

    show c_nikka at midRight with dissolve

    Eu "Mas...{w} o que vocês todas estão fazendo aqui?"

    Nikka "Não é obvio? Nós vamos ao dormitório abandonado"

    Eu "Que, dormitório abandonado?"

    show c_alo at midLeft with dissolve

    Alo "O que Maria? Já tinha se esquecido dele?"

    Eu "Não, mas...{w} eu não sei se isso se encaixa exatamente no que eu chamo de boas ideias?"

    Nikka "Impossível, é um dormitório abandonado, como isso não é uma boa ideia? No mínimo vai ser legal, não?"

    Pensamento "Acho que isso já era meio que esperado dessas três, mas..."

    Eu "O que você acha disso Sophia? E você Hina?"

    Narrador "Você olha para as duas"

    Sophia "Meio que a Nikka me arrastou..."

    Hina "Eu só gosto de andar a noite, me faz bem e aumenta a força da minha {i}Naturalidade{/i}"

    Pensamento "Okay, talvez a Hina tenha mais particularidades do que eu pensei."

    Nikka "Uau, então quer dizer que além da Aneti, você também consegue usar sua {i}Naturalidade{/i}?"

    Nikka "O que ela faz?"

    Hina "Eu consigo produzir essas labaredas. É uma habilidade que existe na minha família, se chama '{i}Fogo de Raposa{/i}'"

    # TODO IMAGEN DA ALO INTERAGINDO COM FOXFIRE

    Alo "Deixa eu ver uma coisa...{w} Uê? Mas não queima?"

    Hina "Não, não queima. Infelizmente o {i}Fogo de Raposa{/i} não serve pra queimar coisa."

    Alo "Eh...{w} Então pra que ele serve?"

    Hina "Bem, basicamente ele serve pra exterminar espiritos malignos, mas principalmente, serve pra iluminar."

    Alo "É, parece bem útil."

    Nikka "Eu também achei, mas..."

    Alo "Mas..."

    Nikka "Quando eu despertar minha {i}Naturalidade{/i} ai sim você vai ver."

    Sophia "Hum...{w} Então você já tem uma ideia de o que vai ser?"

    Hina "Exite uma tendencia na sua família como a minha?"

    Nikka "Não"

    Alo "Então por que você espera esse tipo de coisa da sua {i}Naturalidade{/i}?"

    Nikka "Bem...{w} é porque sou eu.{w} Eu tenho certeza que minha {i}Naturalidade{/i} vai ser algo bem explosivo e taus. Eu vou poder laçar raios de energia, e todo tipo de coisa!"

    Aneti "E alguém da sua família tem alguma Naturalidade do tipo?"

    Nikka "Minha mãe consegue fazer galinha botarem e chocarem ovos mais rápido!"

    Eu "..."

    Eu "......"

    Eu ".........."

    Eu "Aproposito, visto vocês estão chamando todo mundo, onde estão a Serafina e a Olivia?"

    Aneti "Eu tentei chamar a Olivia, mas ela não respondeu mesmo depois de eu bater na porta dela por um...{w} tempo."

    Pensamento "Ela provavelmente tem um sono muito pesado pra conseguir ignorar a Aneti."

    Aneti "Então, visto que ela não respondia, eu entrei no quarto dela assim mesmo."

    Pensamento "Por que eu não estou surpresa?"

    Eu "E então?"

    Aneti "Ela não estava no quarto."

    Hina "Ela provavelmente deve ter saído pra passear ou coisa do tipo."

    Eu "Faz sentido."

    Eu "E a Serafina?"

    Nikka "Com a Serafina foi a mesma coisa, a gente tentou chamar ela, mas ela não estava no quarto."

    Pensamento "Que estranho."

    Nikka "Ela provavelmente deve estar caçando um lugar pra se esconder."

    Eu "Como assim?"

    Sophia "É que um pouco mais cedo teve uma explosão?{w} Um barulho bem alto que veio do quarto dela, agente tentou falar com ela, mas na hora ela falou pra não se preocupar."

    Aneti "E porque vocês só não entraram?"

    Nikka "É que o quarto dela tem um tipo de magia automatica,{w} essa magia impede qualquer um além do dono do quarto de entrar."

    Hina "Uau!"

    Aneti "De fato, uau!"

    Aneti "Até eu queria um sistema de segurança desse."

    Eu "Quer dizer que isso é uma coisa que só rola no quarto dela em toda a academia?"

    Sophia "Sim, a professora disse que foi uma aluna que dormia lá que colocou essa magia."

    Pensamento "...?"

    Eu "Você quer dizer..."

    Sophia "Sim, a aluna que seria a colega de quarto da Serafina."

    Eu "Que complicado, a aluna coloca uma magia dessa no quarto e desaparece."

    Nikka "Sim, a professora disse que já tentou remove-la, mas ela disse que simplesmente não consegue, porque é como se a magia se refize-se depois de um periodo."

    Hina "Você quer dizer que a professora remove a magia mas ela se refaz?"

    Sophia "Exatamente.{w} E como a professora é a unica que sabe usar ela, agente vai ter que esperar até amanhã pra ver o que rolou no quarto dela. Visto que ele só abre pra quem é o dono"

    Eu "Em suma, ela pode ou não estar no quarto dela e ninguém sabe."

    Sophia "Sim, mas agente tem quase certeza de que não está porque a Serafina não é do tipo que consegue ignorar pessoas a chamando."

    Eu "Hum...{w} Será que ela e a Olivia estão juntas?"

    Sophia "Talvez."

    scene bg_i_ceu with slowFade

    Narrador "Depois de conversar mais um pouco, o grupo segue seu caminho"

    $renpy.pause(1)

    scene bg_f_est with dissolve

    # Elas seguem rumo ao rio

    Eu "Agora que eu percebi, a gente está atravessando a ilha inteira."

    show c_aneti at right with dissolve

    Aneti "Sim"

    Eu "É que tipo, a direção pra onde nós estamos indo, está literalmente no oposto do dormitório Azul"

    Aneti "Sim, por que?"

    Eu "É só que foi uma boa caminhada só pra ir chamar eu e a Alo."

    Aneti "Ah, não liga pra isso, não teria graça ir só pra esses lugares."

    show c_nikka at midRight with dissolve

    Nikka "Sim, como eu sempre digo, quanto mais gente melhor!"    

    show c_sophia at midLeft with dissolve

    Sophia "Exato."

    show c_hina at center with dissolve

    Hina "Sim, no pior dos casos, se o destino em si não for tão legal e não pagar a viajem, a companhia vai."

    Eu "Isso é isso é uma forma interessante de ver as coisas."

    scene bg_f_est with dissolve

    $renpy.pause(0.5)

    scene bg_f_flo with slowFade

    show c_aneti at center with dissolve

    Aneti "Vamos ver...{w} a partir daqui eu nunca fui."

    Eu "O dormitório está pra lá?"

    Aneti "Sim"

    Nikka "Mas e a entrada? O caminho acaba aqui."

    Aneti "Quanto a isso, não sei se foi o tempo ou qualquer outra coisa, provavelmente foi a Academia tentando oculta, mas passando por entre algumas arvores"

    Aneti "E indo por perto do rio, nós vamos encontrar a estrada, não é Lemu?"

    Lemu "..."

    show c_alo at midLeft with dissolve

    Alo "Então vamos lá"

    scene bg_f_flo with dissolve

    $renpy.pause(0.5)

    scene bg_i_ceu with fade

    Alo "Nossa, tá dificil aqui, Hina, pode dar uma iluminada aqui?."

    Hina "Pode deixar."

    Alo "Nossa, melhorou bastantes. Valeu"

    scene bg_f_pnt with slowFade

    $renpy.pause(1)

    show c_aneti at center with dissolve

    show c_sophia at right with dissolve

    show c_nikka at midRight with dissolve

    show c_alo at midLeft with dissolve

    show c_hina at left with dissolve

    Aneti "Olha lá, uma ponte!"

    Hina "É parece que já faz um bom tempo que ela não é bem cuidada."

    Sophia "Verdade"

    Alo "Pensa só, nós estamos aqui, sozinhas, sem que ninguém sabendo pra que possam nos resgatar"

    Nikka "Verdade, já pensou se alguma coisa rola com a gente?"

    Sophia "Parem de ser idiotas"

    Pensamento "Tá, eu estou ou pouco assustada, mas  nad..."

    Nikka "Não foi a professora Celia que tinha dito que os alunos desse dormitório abandonado tinha desaparecido?"

    Narrador "Vocês ouvem algo se mexendo na floresta."

    Eu "Ahhhhhh!!!"

    Sophia "Ahhhh!"

    Aneti "O que foi isso?"

    Hina "Eu não sinto nenhuma entidade maligna"

    Lemu "..."

    Aneti "Gente, eu preciso que vocês se acalmem..."

    Alo "..."

    Sophia "..."

    Nikka "..."

    Hina "..."

    Aneti "Gente, pelo que o Lemu me disse, a causa desse barulho foi...{w}O vento."

    Aneti "O que foi?"

    Aneti "Gente, está de noite, qualquer som parece um barulhão."

    Sophia "...{w} ......{w} Tá, de qualquer forma, vamos."

    # Aneti pergunta aos animais sobre o Dormitório

    scene bg_f_pnt with dissolve

    $renpy.pause(0.5)

    scene bg_f_est with dissolve

    $renpy.pause(0.5)

    scene bg_f_bif with dissolve

    Nikka "Olha gente, tem outra estrada"

    Sophia "Verdade"

    Aneti "Hum... {w}entendi...{w} Obrigado Lemu."

    Aneti "Segundo o Lemu o dormitório está pra aquela direção."

    Pensamento "Não sei se a maior maluquice aqui é a Aneti conversar como o suposto Príncipe dos Lêmures ou se é agente que tá seguindo ela que ouve ele..."

    Alo "Então é pra lá né?...{w} Só por curiosidade, o que tem pro outro lado?"

    Aneti "Pelo que eu ouvi dos pássaros, pra lá fica o vulcão..."

    Alo "Só isso?"

    Aneti "Não sei ao certo."

    Alo "...?"

    Aneti "Por algum motivo os pássaros não gostam de ira pra aquela parte da ilha..."

    Nikka "Uuuuu"

    Nikka "Isso é bem interessante, não é?"

    Nikka "Por que será?{w} Será que existe algum tipo de grande mal que infesta aquela área?"

    Sophia "Bem gente, eu acho que é melhor esquecer esse caminho, né? Vamos continuar nosso roteiro padrão."

    Hina "Pra falar a verdade, você não está tão errada Nikka, realmente parece que tem alguma coisa naquela direção."

    Aneti "Você quer dizer que..."

    Hina "Não precisa se preocupar, não é nada de mais..."

    Sophia "Hina, você falando assim só me deixa mais preocupada"

    Hina "Bem, eu acho que tá tudo bem então..."

    Sophia "Não melhorou"

    Pensamento "Definitivamente a Hina não tem um bom tato no que toca acalmar as pessoas"

    Eu "Eu concordo com a Sophia, a gente deveria voltar a nossa programação padrão."

    Aneti "Bem, já que foi votado...{w} então vamos pra lá."

    # O Grupo vai para lá e encontra o dormitório 

    Narrador "O grupo fica de frente pra construção"

    Nikka "Bem... aqui estamos nós, o dormitório abandonado..."

    Alo "Meio decadente, certamente."

    Nikka "Bem, essa é a logica do abandonado no nome"

    Alo "Verdade."

    Sophia "Mas gente, olhando bem... nem está tão ruim assim, na verdade, ele não me parece ter sido abandonado a tanto tempo."

    Eu "Verdade"

    Pensamento "Uê? Cade a Hina?"

    Narrador "Você olha em volta e que que ela tinha parado em determinado ponto e observava o dormitório de longe."

    Pensamento "O que aconteceu?"

    Nikka "Gente! Vamos logo, vamos entrando!"

    Hina "Gente, eu acho melhor agente n..."

    Aneti "Vamos lá Nikka!"

    Alo "Vem Maria."

    Pensamento "Algo está de errado com a Hina..."

    Eu "Aconteceu algo Hina?"

    Hina "Eu estou sentindo algo ruim lá dentro...{w} Algo muito pesado."

    Eu "Algo muito pesado?..."

    Eu "O que isso quer dizer?"

    Hina "Eu...{w} também não tenho certeza, mas... é algo de ruim."

    Eu "Se você está dizendo isso... então eu acho melhor a gente chamar as meninas e..."

    # E no dormitório encontra três garotas desmaiadas

    Narrador "De repente Aneti saiu do dormitório abandonado e veio em nossa direção."

    Aneti "Hina, Sophia, Maria! Vocês precisam ver isso."

    Narrador "As garotas olharam entre si, então seguiram Aneti pra dentro do dormitório"

    Narrador "Ao entrar, você vê três garotas desacordadas no chão do dormitório"

    Hina "Isso é..."

    Nikka "Sim, a gente achou elas aqui quando entramos."

    Eu "Mas quem são elas?"

    Aneti "Eu não sei, nunca as vi."

    Sophia "Será que são alunas?"

    Hina "Talvez..."

    Pensamento "Será que foi isso que a Hina tinha sentido?{w} Bem, isso não importa agora."

    Eu "Como elas estão?"

    Aneti "Elas estão frias...{w} mas estão respirando."

    Sophia "Nós temos que tirar elas daqui...{w} levar elas pra enfermaria!"

    Eu "Sim"

    Alo "Sim, vamos lá"

    Narrador "Alo e Sophia foram rumo a entrada, porém..."

    Hina "PAREM!!!"

    # Som de explosão

    # Uma sombra negra diz que pra tirar elas dali teria que passar por cima dela e todos são rendidos

    Narrador "De repente, assim que Alo e Sophia se aproximaram da porta, o ambiente da sala mudou, o local se tornou gelado uma neblina preencheu o ambiente"

    Narrador "A neblina era negra, preenchia toda a área e tornava a visão impossível."

    Narrador "Você sente um grande peso sobre seu corpo, seus sentidos vão desvanecendo e sua visão se torna uma incógnita..."

    Cosmico "“ø©°nøæßæ→®æðæ/↓→©øµ“→ðæ°°↑ŧøµæ®°→øß°↓©ø®þø°ð°ßŧ®↓→®°→ŧøðøøß°↓µ↓nðø"

    Cosmico "“ø↓þøßß↓→®ß°↓©ø®þø"

    Narrador "Aquele som no meio da densa neblina era incompreensível para seus ouvido, porém a única coisa que você ainda podia ouvir era seus instinto que diziam 'FUJA!'"

    Pensamento "Eu...{w} não consigo me mover..."

    Narrador "Você tenta correr mas seu corpo não se move, você tenta gritar mas sua voz não sai..."

    Cosmico "“ø©~°´° ®→ß→“°ł ©®→ŧæŧ↓®æ µ°ð→ø©®°"

    # Hina salva todos ali librando uma luz
    Narrador "Sua visão vai desvanecendo e se torna escura, porém..."

    # Quando acorda todos estão fora do dormitório
    Narrador "Você sente sua cabeça girar e girar"

    Pensamento "Onde eu estou?..."

    Narrador "Não não sente mais aquela pressão esmagando seu corpo e você consegue sentir a brisa e o ar fresco da floresta."

    # Além de Hina, Estava professora Kae e a Diretora Alice e Olivia
    Kae "Hum...{w} você acordou?"

    Eu "Professora Kae...?"

    Kae "Sim, sou eu?"

    Alice "Há, Maria! Que bom que acordou..."

    Eu "Diretora...?"

    Alice "Exato?"

    Alice "Maria...{w} sei que você só acabou de acordar, mas que estou muito curiosa pra pra saber o que você e suas amigas estava fazendo aqui..."

    Eu "No..."

    Alice "Não fale, descanse. Amanhã nós falamos sobre isso."

    # Alguns foram dali para a Enfermaria

    Narrador "Depois daquilo você perdeu a consciência novamente."

    # Conversa na enfermaria

    Desconhecido "Maria...{w}Maria..."

    Eu "?"

    Desconhecido "Ela acordou gente!"

    Eu "Alo?"

    Aneti "Maria!"

    Eu "Aneti?"

    Sophia "Que bom que acordou Maria"

    Eu "Sophia"

    Nikka "Que bom que você acordou"

    Eu "Nikka..."

    Narrador "Você suspira em alivio"

    Narrador "Vendo seu suspiro, todas as garotas ficam mais seiras"

    Sophia "Maria...{w} você também...{w} sabe...{w} aquela coisa no dormitório abandonado?"

    Eu "...{w}Sim"

    Eu "Eu fiquei muito assustada com...{w} aquilo..."

    Eu "Mas... estou feliz que todas vocês todas estão bem."

    Narrador "Um silencio correu."

    Eu "O que foi?"

    Aneti "Maria...{w} a Hina..."

    Eu "A Hina?{w} O que aconteceu com ela?"

    Aneti "Ela está em coma e segundo a professora Kae ela não vai acordar tão cedo."

    Eu "O que...?"

    Aneti "A..."

    Alice "Vocês acordaram então?"

    Eu "Diretora?"

    Alice "Venham todas a minha sala, eu quero falar sobre o que aconteceu."

    Nikka "Mas a Maria acabou de acorda!"

    Alice "Isso não me importa, vocês estava muito bem ontem pra ficar explorando onde não deviam."

    Alice "Se não quiserem ser chutadas dessa Academia, venham agora."

    Narrador "Todas olham entre si então seguem a diretora"

    # ---- Na diretoria

    Alice "Primeiro de tudo, eu gostaria de perguntar o que vocês estava fazendo naquela área da ilha?"

    Alice "Se me lembro bem, logo no primeiro dia eu havia avisado para ficarem longe do noroeste da ilha."

    Aneti "Professora...{w}Isso foi minha culpa... foi eu quem chamou todo mundo pra ir lá."

    Aneti "Quanto a Punição...{w} Por favor...{w} Eu sumo toda a culpa..."

    Alo "Não..."

    Sophia "Deforma nenhuma Aneti..."

    Nikka "Nós fomos lá por vontade própria"

    Eu "Sim, você não arrastou ninguém até lá ou coisa do tipo"
    
    Alice "Ainda bem que vocês entendem isso."

    Alice "A punição de vocês quatro será ajudar na limpeza da cantina depois das aulas."

    Eu "Ha???"

    Alo "Que?"

    Sophia "Como?"

    Aneti "..."

    Alice "Deixe-me esclarecer uma coisa a vocês. Vocês não vão ser tratadas como crianças aqui na minha academia."

    Alice "O status de alunas dão a vocês a possibilidade de entrar na academia e estudar aqui, nada mais."

    Alice "Se vocês resolverem ir embora de repente daqui, problema seus, se vocês forem se matar indo ao vulcão é problemas seus, entenderam?"

    Alice "Bem, de qualquer forma, eu chamei vocês aqui pra falar sobre aquelas três garotas em coma na enfermaria"

    Alice "Eu queria saber onde vocês as encontraram?"

    Aneti "Nós as encontramos no dormitório abandonado...{w} Quando entramos elas já estavam lá caídas no chão..."

    Eu "Você sabe quem são diretora?"

    Alice "É claro que sei, só entra e encontra no mar essa academia as pessoa quem eu aceito como alunas aqui."

    Sophia "Isso quer dizer..."

    Alice "Sim, aquelas são três alunas dessa academia."

    Aneti "Mas o que elas estavam fazendo no dormitório abandonado?"

    Alice "Isso que eu queria saber de vocês."

    # A três que foram achadas no dormitório e estão todas em coma

    Alice "Aquelas três: 'Milena', 'Verânia' e 'Aya'."

    Alice "Estavam todas na lista de desparecidos da academia até ontem."

    Pensamento "Desaparecidas?"

    Sophia "Isso quer dizer que elas sairão da ilha escondidas e entraram aqui sem ninguém saber pra ir no dormitório abandonado?"

    Alice "Isso é tolice e também o que mais me intriga."

    Alice "Como disse, ninguém entra ou sai da academia sem que eu saiba,"

    Alice "eu tenho o poder de conceder o revogar a permissão de alguém atravessar a barreira que cerca a ilha."

    Nikka "Você está dizendo que até ontem elas tinham simplesmente...{w} Sumido?"

    Narrador "..."

    Sophia "Mas só pra saber Diretora, há quanto tempo exatamente aquelas três estão desaparecidas?"

    Alice "...{w}Vai fazer aproximadamente...{w} 4 anos..."

    Eu "Que? Mas então como..."

    Alice "Eu também não sei, mas se vocês não sabem de nada então não há mais o que conversar."

    Aneti "Mas Diretora... e quanto a Hina?"

    Alice "Pelo que eu ouvi do estado dela, ela vai ficar em coma por tempo indeterminado. Vai tudo depender da sorte dela."

    Alice "Agradeçam, por que vocês deram uma sorte que ela não teve."

    Eu "Será..."

    Sophia "Maria?"

    Eu "A Hina ontem...{w} Ela parecia saber o que ia acontecer... {w}Sobre aquela coisa que atacou agente."

    Alo "Verdade, eu lembro dela ter gritado naquela hora."

    Alice "Do que vocês estão falando?"

    Nikka "Nós não tivemos chance de falar até agora diretora, mas ontem nós fomos atacadas por uma 'coisa' no dormitório abandonado."

    Eu "Sim, a Hina até antes de encontrar as três garotas, ela tinha me dito que sentia algo ruim por ali"

    Alice "Então...?"

    Alo "Diretora, como tínhamos dito, quando entramos no dormitório abandonado encontramos as três, mas quando formos buscar ajuda alguma coisa atacou a gente"

    Narrador "O olhar de Alice duvidava do que ouvia"

    Alice "Tá, e...{w} como era essa 'Coisa'?"

    Narrador "As meninas se entre olharam, mas demoraram a falar novamente"

    Eu "Eu... não conseguir ver a coisa devido a uma nevoa que encheu toda minha visão"

    Sophia "Foi a mesma coisa comigo, eu só consegui ver vultos."

    Alo "Eu também, além disso a coisa falava mas eu não entedia nada do que ela falava..."

    Aneti "Sim, ela falava em uma linguagem esquisita, impossível de entender até os sons."

    Narrador "A diretora olhou as quatro com olhar cético"

    Alice "Então quer dizer que vocês três forma atacadas por essa 'Coisa', certo?"

    Eu "Sim. E talvez seja ela que esteja também por trás do desparecimento dessas três garotas"

    Narrador "A Diretora deu um suspiro"

    Alice "Deixe-me ver, vocês querem que eu acredite quem uma coisa que vocês não ouviram, tocaram ou viram está por trás de tudo isso?."

    Sophia "..."

    Alo "..."

    Aneti "..."

    Eu "..."

    Alice "Deixe-me repetir algo de falei no primeiro dia de vocês aqui na academia."

    Alice "O motivo daquela área não ser mais usada é porque lá há gases tóxicos,"

    Alice "gases esses que podem causar alucinações como vocês pelo visto experimentaram ou coma...{w} como a amiga de vocês está experimentando."

    Alice "De qualquer forma. Estão todas dispensadas, voltem pra enfermaria... ou vão pra onde quiserem."

    Eu "Mas Diret..."

    Alice "Dispensadas, não ouviu Maria?"

    Eu "Eu... ouvi..."

    Alice "Aproposito, vocês deveriam ira fazer com sua colega, a Olivia."

    Aneti "A Olivia?"

    Alice "Sim, foi ela quem chamou todo mundo pra socorrer vocês quando estavam lá no dormitório abandonado"

    Eu "Mas como ela sabia que nós estamos lá?"

    Alice "Isso é algo que vocês tem que falar com ela. Agora, xó, vão embora daqui."

    return
